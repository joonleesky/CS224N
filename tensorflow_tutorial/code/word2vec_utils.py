...
def sub_sample(words, threshold):
    """Subsample data in respect to 1 - sqrt(threshold/freq(word))"""
    
    word_frequency = Counter(words)
    p_drop = {}
    for word, frequency in word_frequency.items():
        p_drop[word] = (1 - np.sqrt(threshold / (frequency / len(words))))
    
    return [word for word in words if random.random() > p_drop[word]]
...
def batch_gen(download_url, expected_byte, vocab_size, batch_size, 
                skip_window, visual_fld, threshold):
    ...
    print('total words:',len(index_words))
    index_words = sub_sample(index_words, threshold)
    print('subsampled words: ',len(index_words))
