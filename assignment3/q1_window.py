...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i, label in enumerate(labels):
            window = []
            window_t = []
            if i-window_size >= 0 and i+window_size < sen_size:
                window = sentence[i-window_size:i+window_size+1]
            elif i-window_size < 0 and i+window_size < sen_size:
                window = [start] * (window_size - i) + sentence[:i+window_size+1]
            elif i-window_size > 0 and i+window_size >= sen_size:
                window = sentence[i-window_size:] + [end] * (window_size+i+1-sen_size)
            else:
                window = [start] * (window_size - i) + sentence + [end] * (window_size+1+i-sen_size)    
            
            for word in window:
                for feat in word:
                    window_t.append(feat)
                    
            windowed_data.append((window_t, label))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, [-1, n_window_features * embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W", shape=[n_window_features * embed_size, hidden_size],
                                    initializer=tf.contrib.layers.xavier_initializer())
        U = tf.get_variable("U", shape=[hidden_size, n_classes],
                                    initializer=tf.contrib.layers.xavier_initializer())
        ...
        pred = tf.nn.relu(tf.matmul(h_drop, U) + b2)
