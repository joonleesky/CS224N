...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        new_sequence = sentence[:max_length] if len(sentence) >= max_length else sentence + [zero_vector] * (max_length - len(sentence))
        new_label = labels[:max_length] if len(labels) >= max_length else labels + [zero_label] * (max_length - len(labels))
        mask_sequence = [True] * max_length if len(sentence) >= max_length else [True] * len(sentence) + [False] * (max_length - len(labels))
        ret.append((new_sequence, new_label, mask_sequence))
### END YOUR CODE ###  
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedding_matrix = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder)
        embeddings = tf.reshape(embeddings, [-1, self.max_length, n_features * embed_size])
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable("U", shape = [Config.hidden_size, self.config.n_classes], 
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        _loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = self.labels_placeholder, logits = preds)
        loss = tf.reduce_mean(tf.boolean_mask(_loss, self.mask_placeholder))
        ### END YOUR CODE
        return loss
